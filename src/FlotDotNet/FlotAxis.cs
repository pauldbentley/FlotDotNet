namespace FlotDotNet
{
    ...
    public sealed class FlotAxis
    {
        ...
        /// </summary>
        [JsonIgnore]
        public List<FlotCategory> Categories { get; set; } = new List<FlotCategory>();
        ...
        private object CategoriesObject
        {
            get
            {
                if (Categories?.Count > 0)
                {
                    // can be an array of just names
                    // if there are any without a value
                    // we output as an array
                    if (Categories.Any(c => !c.Value.HasValue))
                    {
                        return Categories.Select(c => c.Name);
                    }

                    // can be a dictionary of name values
                    return Categories.ToDictionary(k => k.Name, v => v.Value);
                }
